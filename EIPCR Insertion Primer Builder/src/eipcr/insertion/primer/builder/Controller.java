...
//        System.out.println("revcomp:" + toReturn);
    ...
    public void generatePrimers() {
        int oligoNumber = 3; //counter for naming the oligos
...
//                System.out.println("end revHomology: " + reverseHomology);
                ...
                String nameStart2 = Integer.toString(oligoNumber + 1);

                while (nameStart.length() < 3) {
                    nameStart = "0" + nameStart;
                }
                while (nameStart2.length() < 3) {
                    nameStart2 = "0" + nameStart2;
                }
                output.add(_nameRoot + nameStart);
                output.add(forwardPrimer);
                output.add("#" + (j - 2) +" forward primer for " + insertionSiteNames.get(i) + " | Tm=" + calcTempBasic(forwardPrimer) + " Celcius");
                output.add(_nameRoot + nameStart2);
                output.add(reversePrimer);
                output.add("#" + (j - 2) +" reverse primer for " + insertionSiteNames.get(i) + " | Tm=" + calcTempBasic(reversePrimer) + " Celcius");
...
//                output.add(reversePrimer);
        ...
        try {
            ...
            if (outFile != null) {
                ...
                outWriter.write("Name,Sequence,Note");
                outWriter.newLine();
                for (int i = 0; i < output.size(); i += 3) {
                    outWriter.write(output.get(i) + "," + output.get(i + 1) + "," + output.get(i + 2));
                    outWriter.newLine();
                }
            }
        } catch (IOException ex) {
